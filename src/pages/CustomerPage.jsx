...
export default function CustomerPage() {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.detail ||
          "Could not find any booking for that vehicle",
      );
    } finally {
      setBusy(false);
    }
  ...
  useEffect(() => {
    if (initial) lookup(initial); /* eslint-disable-next-line */
  }, []);
  ...
  const approve = async () => {
    setBusy(true);
    try {
      await api.post(`/bookings/${active.id}/approve`, {
  ...
  const sendWhatsappBill = async () => {
    setBusy(true);
    try {
      const r = await api.post(`/track/${active.id}/send-bill`, {
        plate_number: active.plate_number,
      });
    ...
    <div className="min-h-screen bg-zinc-100 text-zinc-900">
      <div className="bg-black text-orange-500 py-1.5">
        <Marquee speed={40} gradient={false}>
          <span className="font-mono text-[10px] uppercase tracking-[0.3em] mx-8">
            TRACK YOUR CAR · NO LOGIN · WHATSAPP BILL · LIVE STATUS · YOUR CAR
            OUR PRIORITY ·
          </span>
          <span className="font-mono text-[10px] uppercase tracking-[0.3em] mx-8">
            TRACK YOUR CAR · NO LOGIN · WHATSAPP BILL · LIVE STATUS · YOUR CAR
      ...
      <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-xl border-b border-zinc-200">
        <div className="max-w-3xl mx-auto px-5 py-4 flex items-center justify-between">
          ...
          >
            <MacJitLogo size={32} />
            <span className="font-display font-black text-lg tracking-tighter group-hover:text-orange-500">
              MACJIT
            </span>
          </button>
          <button
            onClick={() => nav("/")}
            className="flex items-center gap-1 font-mono text-[10px] uppercase tracking-widest text-zinc-500 hover:text-orange-500"
          >
      ...
      <main className="max-w-3xl mx-auto px-5 py-8 space-y-8 pb-20">
        <div>
          ...
          <p className="font-mono text-xs text-zinc-500 mt-2">
            Enter the number plate of the car you dropped off.
          </p>
        </div>
        ...
        {searched && !active && !busy && (
          <div
            data-testid="no-booking"
            className="border border-dashed border-zinc-300 p-12 text-center bg-white"
          >
            ...
            <p className="text-sm text-zinc-500 mt-1">
              Double-check the plate number, or visit the reception desk.
            </p>
          </div>
        )}
        ...
        {active && (
          <>
            <section
              className="bg-white border border-zinc-200 p-6 shadow-[0_8px_32px_rgba(0,0,0,0.04)]"
              data-testid="active-booking-card"
            >
              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  ...
                  <h2 className="font-display font-black text-2xl mt-0.5">
                    {active.car_make} {active.car_model}
                  </h2>
                  <p className="font-mono text-sm text-zinc-600 mt-0.5">
                    {active.plate_number} · {active.service_type}
                  </p>
                ...
                <div className="mt-3 pt-3 border-t border-zinc-100 grid grid-cols-2 gap-4 font-mono text-sm">
                  <div data-testid="assigned-mechanic">
                    ...
                    {active.mechanic_phone && (
                      <a
                        href={`tel:${active.mechanic_phone}`}
                        className="text-[11px] text-orange-600 flex items-center gap-1 mt-0.5"
                      >
              ...
              {active.estimated_start_at && active.status !== "PAID" && (
                <div
                  ...
                  className="mt-3 bg-black text-white p-3 flex items-center justify-between"
                >
                  <div>
                    ...
                    <p className="font-display font-bold text-sm">
                      {new Date(active.estimated_start_at).toLocaleString([], {
                        weekday: "short",
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                      {" → "}
                      {active.estimated_end_at &&
                        new Date(active.estimated_end_at).toLocaleString([], {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                    </p>
            ...
            {active.approval_pending && (
              <section
                ...
                className="bg-yellow-50 border-2 border-orange-500 p-6"
              >
            ...
            {active.status === "BILLED" &&
              ...
              (() => {
                ...
                const sub = Number(
                  active.subtotal || billTotal + Number(active.discount || 0),
                );
                const base = Math.max(
                  0,
                  sub - itemsSum - Number(active.extra_cost || 0),
                );
                const svc = String(active.service_type || "service").replace(
                  /-/g,
                  " ",
                );
                return (
                  ...
                  >
                    ...
                    <div
                      className="mt-3 divide-y divide-white/10 font-mono text-sm"
                      data-testid="customer-bill-breakdown"
                    >
                      ...
                      {Number(active.discount || 0) > 0 && (
                        <div className="flex justify-between py-2 text-emerald-400">
                          <span>
                            {active.loyalty_tier || ""} loyalty (
                            {active.discount_pct || 0}%)
                          </span>
                    ...
                    <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-orange-500 mt-5">
                      Total due
                    </p>
                    ...
                    <div className="flex flex-wrap gap-3 mt-5">
                      ...
                      >
                        <MessageCircle className="w-4 h-4" /> Send bill on
                        WhatsApp
                      </button>
                    </div>
                  </section>
            ...
            {active.paid && (
              <div
                ...
                className="bg-emerald-500 text-white p-8 text-center"
              >
                <p className="font-mono text-[10px] uppercase tracking-[0.3em]">
                  Thank you for visiting
                </p>
                <p className="font-display font-black text-3xl mt-1 tracking-tighter">
                  Drive safe! 🏁
                </p>
                {data.invoice_url && (
                  <a
