...
export default function LandingPage() {
  ...
  const { user } = useAuth();
  const goLogin = () => nav(user ? `/${user.role}` : "/login");
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <nav className="sticky top-0 z-30 bg-zinc-950/80 backdrop-blur-xl border-b border-zinc-900">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <button data-testid="nav-login" onClick={goLogin} className="bg-orange-500 hover:bg-orange-400 text-black font-display font-black uppercase tracking-widest px-4 py-2 transition-colors flex items-center gap-2">
            {user ? "Dashboard" : "Sign In"} <ArrowRight className="w-3 h-3" />
          </button>
        </div>
      </nav>
      ...
      <footer id="contact" className="border-t border-zinc-900 bg-black">
        ...
        <div className="border-t border-zinc-900 py-4">
          <p className="max-w-7xl mx-auto px-6 font-mono text-[10px] uppercase tracking-[0.3em] text-zinc-600">© 2026 MacJit · macjit.com · All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
